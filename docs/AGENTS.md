...
An open-source, metadata-driven pipeline framework: a design-time compiler that validates a YAML pipeline definition and generates native Airflow DAGs, dbt projects, Meltano configs, and Flink job manifests — then gets out of the way. It is **not** a runtime. Full spec: `docs/framework-spec.md`.
...
- **Bronze never rejects records.** Quarantine and validation happen in Silver, not Bronze. This was a deliberate, hard-won correction earlier in the design — don't reintroduce rejection logic into Bronze ingestion code.
...
Check `docs/decisions.md` first — most edge cases (row-count expectations across zones, job-vs-view-vs-materialized-view, CDC-as-mirror-vs-log, streaming-vs-batch reconciliation) were already reasoned through and the rationale is there, not just the conclusion. If it's genuinely new, make the smallest decision consistent with the constraints above and note it — don't silently pick a direction that contradicts an existing entry.
